...
- [ ] Docs (`.claude/CLAUDE.md`, `docs/*.md`, `README*.md`)
- [ ] Eval (`eval/run.sh`)
...
- [ ] New/changed hooks are wired into `hooks/hooks.json` and, if blocking, justified
      (low false-positive trigger — see `.claude/CLAUDE.md` hook profile policy)
...
- [ ] `.claude/CLAUDE.md` updated if agent/hook/skill structure changed (never add a
      `CLAUDE.md` at the plugin root — it ships without being loaded and fails `--strict`)
- [ ] `.claude-plugin/plugin.json` version bumped if this PR changes **any file that ships
      in the plugin tree** — agents, hooks, skills, `eval/`, `.claude/CLAUDE.md`, `README*`, `docs/`
