...
| PPT/기획서 → PDF 변환 후 처리 | `fe-vision` — PDF 변환된 슬라이드에서 화면별 추출 (직접 .pptx 읽기 불가 — PDF 변환 필요) |
...
## 화면 흐름
- 화면 목록: (화면ID — 화면명 — 충실도 등급: 고증 시안/와이어프레임/스케치)
...
// AskUserQuestion 호출 — 단일 선택(라디오)
{
  "questions": [{
    ...
    "multiSelect": false,
    "options": [
      { "label": "풀 자동",   "description": "fe-start 파이프라인 전체 실행 (구현→리뷰→커밋·PR)" },
      { "label": "구현만",   "description": "fe-build 만 수행하고 멈춤 (리뷰·커밋·PR 없음)" },
      { "label": "스펙 수정", "description": "feature.md 수정 후 이 단계를 다시 묻는다" }
    ]
  }]
}
```
...
선택 결과에 따라 분기한다:
- 풀 자동 → 이 선택이 곧 "구현 시작" 승인이다. fe-start 파이프라인으로 이어가되, Phase 0(feature.md 읽기·코드베이스 분석)은 실행하고 Phase 1 STOP(구현 시작 확인)은 생략하여 Phase 2 로 진행한다. 남는 사람 개입은 Phase 5("커밋할까요?") 한 번뿐.
- 구현만 → `/fe-rail:fe-build` 스킬을 이어서 실행하고 멈춘다 (리뷰·커밋·PR 없음). feature.md 경로를 컨텍스트로 전달한다.
- 스펙 수정 → 사용자가 "Other"로 수정 내용을 이미 입력했으면 그대로 반영한다. 선택지만 고른 경우 "무엇을 수정할까요?" 를 텍스트로 추가 질문한 뒤 답변을 받아 feature.md 를 수정하고 이 Phase 를 다시 묻는다.
